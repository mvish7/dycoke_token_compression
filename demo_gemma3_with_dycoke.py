...
class Gemma3Inference:
    """Wrapper for running inference with Custom Gemma3 model."""
    ...
    def __init__(self, model_id: str, dycoke_args: dict):
        ...
        self.config = CustomGemma3Config.from_pretrained(model_id)
        self._apply_dycoke_settings(dycoke_args)

        if dycoke_args["use_dycoke"]:
            self.model = CustomGemma3ForConditionalGeneration.from_pretrained(
                model_id,
                config=self.config,
            ...
            # load original Gemma3 from transformers
    ...
    def _apply_dycoke_settings(self, dycoke_args):
        ...
        if dycoke_args["use_dycoke"]:
            ...
            self.config.dycoke_num_tokens_per_frame = 256
    ...
    def prepare_inputs(self, prompt: str, video_path: str, fps: float = 1.0,
        ...
        Args:
            ...
            sampling_factor (int): selects every Nth frame from the video
...
def parse_args():
    ...
    parser.add_argument("--model_id", type=str, default="google/gemma-3-4b-it", help="Model ID.")
...
def main():
    ...
    inference = Gemma3Inference(args.model_id, {"use_dycoke": args.use_dycoke, "dycoke_l": args.dycoke_l,
                                                "dycoke_p": args.dycoke_p, "dycoke_k": args.dycoke_k})
    inputs, input_len = inference.prepare_inputs(args.prompt, args.video, fps=args.fps, sampling_factor=args.sampling_factor)

    generation, gen_time, mem_used = inference.generate(
        inputs, input_len, max_new_tokens=args.max_tokens
    )
