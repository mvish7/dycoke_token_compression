...
from .configuration_qwen2_5_vl import CustomQwen2_5_VLConfig, CustomQwen2_5_VLTextConfig, CustomQwen2_5_VLVisionConfig
...
class DycokeConfigs:
    ...
    def __init__(self):
        ...
        self.dycoke_num_tokens_per_frame = 360
...
class Qwen2_5_VLVisionAttention(nn.Module):
    def __init__(self, config: CustomQwen2_5_VLVisionConfig) -> None:
        super().__init__()
...
class Qwen2_5_VLPreTrainedModel(PreTrainedModel):
    config_class = CustomQwen2_5_VLConfig
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    config_class = CustomQwen2_5_VLVisionConfig
...
class Qwen2_5_VLRotaryEmbedding(nn.Module):
    def __init__(self, config: CustomQwen2_5_VLTextConfig, device=None):
        super().__init__()
...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def __init__(self, config: CustomQwen2_5_VLTextConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class Qwen2_5_VLDecoderLayer(GradientCheckpointingLayer):
    def __init__(self, config: CustomQwen2_5_VLTextConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
@auto_docstring
class CustomQwen2_5_VLTextModel(Qwen2_5_VLPreTrainedModel):
    config_class = CustomQwen2_5_VLTextConfig

    def __init__(self, config: CustomQwen2_5_VLTextConfig, dycoke_config: DycokeConfigs = None):
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if self.dycoke_configs.dycoke:
                self.dycoke_configs.seq_length_with_past = seq_length + past_key_values_length
                if layer_idx < self.dycoke_configs.dycoke_l:
                    past_key_values.kv_cache = None
                elif layer_idx == self.dycoke_configs.dycoke_l and past_key_values.kv_cache is None and position_ids.shape[1] == 1:
                    past_key_values.dycoke_pruning(layer_outputs, layer_idx, self.dycoke_configs)
...
@auto_docstring
class CustomQwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    base_model_prefix = ""
    ...
    def __init__(self, config):
        super().__init__(config)
        # DyCoke parameters
        dycoke = getattr(config, "dycoke", False)
        if dycoke:
        ...
        self.language_model = CustomQwen2_5_VLTextModel._from_config(config.text_config, dycoke_config=self.dycoke_configs)
        ...
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                ...
                if self.dycoke_configs.dycoke:
                    ...
                    vid_tokens_mask = torch.where(input_ids == self.config.video_token_id, 1, 0)
                    vid_tok_start, vid_tok_end = torch.where(vid_tokens_mask > 0)[1][0] - 1, \
                                                 torch.where(vid_tokens_mask > 0)[1][-1] + 1
                    new_vid_toks = torch.tensor(self.config.video_token_id, dtype=torch.int64).repeat(
                        video_embeds.shape[0]).unsqueeze(0)
                    new_input_ids = torch.cat((input_ids[:, :vid_tok_start.item()], new_vid_toks.to(input_ids.device),
                                               input_ids[:, vid_tok_end.item():]), dim=1).to(input_ids.dtype)
                    input_ids = new_input_ids

                    # assign vid_token_start_id to dycoke configs, as original dycoke uses "image_token_start_id" we are
                    # keeping the nomenclature same
                    self.dycoke_configs.image_token_start_index = vid_tok_start.item()

                    # input_embeds also needs to be adapted as input_ids contain less video_tokens
                    inputs_embeds = self.get_input_embeddings()(input_ids)

                    # attention mask also considers old dimension of input_ids, hence needs adaptation
                    attention_mask = torch.ones_like(input_ids)

                    # cache_position also needs to be adapted as it considers old input_ids
                    past_seen_tokens = past_key_values.get_seq_length() if past_key_values is not None else 0
                    cache_position = torch.arange(
                        past_seen_tokens, past_seen_tokens + inputs_embeds.shape[1], device=inputs_embeds.device
                    )
...
class CustomQwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
...
__all__ = ["CustomQwen2_5_VLForConditionalGeneration", "CustomQwen2_5_VLModel",
           "Qwen2_5_VLPreTrainedModel", "CustomQwen2_5_VLTextModel"]
