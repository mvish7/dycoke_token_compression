...
class DycokeConfigs():
    ...
    def __init__(self):
        ...
        self.dycoke_num_tokens_per_frame = 256
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        if use_cache and not self.training:
            ...
            past_key_values_length = past_key_values.get_usable_length(seq_length)  # past_key_values.get_seq_length()
        ...
        **lm_kwargs,
        ...
        if pixel_values is not None:
            ...
            if self.dycoke_configs.dycoke:
                ...
                self.dycoke_configs.image_token_length = image_features.shape[0] - 1

                # after dycoke temporal pruning, reduce the image_tokens from input_ids
                img_ids_mask = torch.where(input_ids==self.config.image_token_id, 1, 0)
                img_tok_start, img_tok_end = torch.where(img_ids_mask>0)[1][0]-1, torch.where(img_ids_mask>0)[1][-1]+1
                new_img_toks = torch.tensor(self.config.image_token_id, dtype=torch.int64).repeat(image_features.shape[0]).unsqueeze(0)
                new_input_ids = torch.cat((input_ids[:, :img_tok_start.item()], new_img_toks.to(input_ids.device),
                                           input_ids[:, img_tok_end.item():]), dim=1).to(input_ids.dtype)
                input_ids = new_input_ids

                self.dycoke_configs.image_token_start_index = img_tok_start.item()
        ...
        if not isinstance(causal_mask_mapping := attention_mask, dict) and self.dycoke_configs.dycoke:
            ...
            # altering token_type_ids: it contains 0s for text tokens and 1s for image tokens. token_type_ids should
            # have same dimensions as input_ids

            token_type_ids = torch.where(input_ids == self.config.image_token_id, 1, 0).to(torch.int64)
            ...
            # Create the masks
            causal_mask_mapping = {
                "full_attention": create_causal_mask(config=mask_kwargs["config"],
                                                     input_embeds=mask_kwargs["input_embeds"],
                                                     attention_mask=None,
                                                     cache_position=mask_kwargs["cache_position"],
                                                     past_key_values=mask_kwargs["past_key_values"]),

                "sliding_attention": create_sliding_window_causal_mask(config=mask_kwargs["config"],
                                                     input_embeds=mask_kwargs["input_embeds"],
                                                     attention_mask=None,
                                                     cache_position=mask_kwargs["cache_position"],
                                                     past_key_values=mask_kwargs["past_key_values"]),
...
class CustomGemma3ForConditionalGeneration(Gemma3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: Gemma3Config):
        ...
        self.dycoke_l = getattr(config, "dycoke_l", 3)
        self.dycoke_p = getattr(config, "dycoke_p", 0.8)
        self.dycoke_num_tokens_per_frame = getattr(config, "dycoke_num_tokens_per_frame", 196)
        self.dycoke_k = getattr(config, "dycoke_k", 0.3)
        self.image_token_start_index = getattr(config, "image_token_start_index", 14)
        self.post_init()
